...
import { Dumbbell, Flame, Droplets, Scale, TrendingUp, Activity, ChevronRight, Target } from 'lucide-react'
...
export default function Dashboard() {
  ...
  // Current week workout completion
  ...
  const plannedDays = weekDays.filter(d => {
    const dow = new Date(d).getDay()
    return workoutPlan.schedule?.[dow]
  }).length
  ...
  // Streak — consecutive days with any log entry (workout or meal)
  ...
  const calPct = Math.min(100, Math.round((todayCalories / nutritionGoals.calories) * 100))
  const protPct = Math.min(100, Math.round((todayProtein / nutritionGoals.protein) * 100))
      ...
      {todayWorkout ? (
        <Link to="/workout" className="block">
          <div className={`rounded-2xl p-5 border ${todayLogged ? 'bg-green-500/10 border-green-500/30' : 'bg-gray-900 border-gray-800 hover:border-gray-700'} transition-colors`}>
            ...
            {todayLogged && (
              <p className="mt-3 text-green-400 text-sm font-medium">Completed today</p>
            )}
          </div>
        </Link>
      ) : (
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <KPICard
            icon={Dumbbell}
            label="Workouts"
            value={`${completedDays} / ${plannedDays}`}
            sub="sessions done"
            color="text-blue-400"
            to="/log"
          />
        ...
        <div className="grid grid-cols-2 gap-3">
          <KPICard
            icon={Flame}
            label="Calories"
            value={todayCalories ? `${todayCalories}` : '–'}
            sub={`goal: ${nutritionGoals.calories} kcal`}
          ...
          <KPICard
            icon={Target}
            label="Protein"
            value={todayProtein ? `${todayProtein}g` : '–'}
            sub={`goal: ${nutritionGoals.protein}g`}
          ...
          <div className="card space-y-3">
            <div>
              <div className="flex justify-between text-xs mb-1.5">
                ...
                <span className="text-gray-500">{calPct}%</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-xs mb-1.5">
                ...
                <span className="text-gray-500">{protPct}%</span>
              </div>
              <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
                <div className="h-full bg-yellow-400 rounded-full transition-all" style={{ width: `${protPct}%` }} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <KPICard
            ...
            sub={weightDelta !== null ? `${weightDelta > 0 ? '+' : ''}${weightDelta} from last` : 'no data yet'}
              ...
              <div key={i} className="flex flex-col items-center gap-1">
                ...
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium border
                  ${done ? 'bg-green-500 border-green-500 text-black' :
                    isToday && planned ? 'border-green-500 text-green-400' :
                    planned ? 'border-gray-700 text-gray-600' :
