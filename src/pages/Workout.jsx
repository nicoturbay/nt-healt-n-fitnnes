...
function ExerciseCard({ exercise, onLog }) {
  ...
  return (
    <div className={`flex flex-col rounded-2xl overflow-hidden border transition-all h-full ${
      logged ? 'border-green-500/50 bg-green-500/5' : 'border-gray-800 bg-gray-900'
    }`}>
      {/* Category header */}
          ...
          {sets.map((set, i) => (
            <div key={i} className="grid grid-cols-12 gap-1 items-center">
              ...
              <input
                ...
                type="number"
                placeholder={exercise.targetReps.toString().split('–')[0]}
                value={set.reps}
...
export default function Workout() {
  ...
  const sessionComplete = workoutLogs.some(w => w.date === todayStr && w.workoutKey === displayKey)
      ...
      <div className="space-y-4">
        ...
        <div className="card flex flex-col items-center py-16 text-center">
          ...
          <p className="text-gray-500 text-sm mt-2 max-w-xs">
            Send Clawckie your gym equipment and goals to get your custom plan.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          {loggedCount > 0 && !sessionComplete && (
            <span className="text-xs text-gray-400 bg-gray-800 px-3 py-1.5 rounded-full">
              {loggedCount}/{totalExercises} logged
            </span>
          )}
          {sessionComplete && (
          ...
          {!sessionComplete && loggedCount > 0 && (
            <button
              ...
              className="bg-green-500 text-black font-bold text-sm px-4 py-2 rounded-xl hover:bg-green-400 transition-colors"
            >
              Finish Workout
            </button>
          )}
      ...
      {/* Finish CTA at bottom if needed */}
      {!sessionComplete && loggedCount > 0 && (
        <button
          onClick={finishWorkout}
          className="w-full py-3 rounded-2xl bg-green-500 text-black font-bold hover:bg-green-400 transition-colors"
        >
          Finish Workout ({loggedCount}/{totalExercises} exercises logged)
        </button>
      )}
